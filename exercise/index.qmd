...
| **C6HJR (you)** | **R + Quarto** | **t = −3.80** | **270** | **−0.242** |
...
**Goal:** choose ONE deviation from the [specification menu](spec-menu.qmd), preregister your choice, run the model, and report the result in the standard format.

**Preregistration (Track B):** commit your choice *before* running the model. The commit message must follow the format:

```
prereg: <DV> ~ <IV> | <estimator> | <sample>
```

For example: `prereg: mposframe ~ unemp | twoway FE | all years`

**Preregistration (Track A):** post your intention to the [Discussion thread](https://github.com/CodeMoreh/applied-replication/discussions) *before* running the model, then run it.

**After running:** paste your result into the Discussion thread using the standard result line (the `report_result()` helper in your template, or the format on the [Specification menu](spec-menu.qmd) page, produces this for you automatically).

**Evidence artifact:** a timestamped commit (Track B) or Discussion post (Track A) showing the prereg precedes the result; the result line in the Discussion thread.
...
Pick a second specification from the menu, or compare your fixed-effects result with random effects using `plm()`'s Hausman test, or weight country-years by `n_cy`. Report a second result line to the thread.
...
- includes a standard result line in the Discussion thread (the format is on the [Specification menu](spec-menu.qmd) page), and
