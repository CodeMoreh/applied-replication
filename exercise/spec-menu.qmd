...
One note on ordering. The day runs Multi100's own sequence in reverse: its analysts chose freely first and were standardised months later, while you start from the standardised baseline model and recover the freedom one declared axis at a time. The menu therefore expands a single constrained option rather than restricting a blank page.
...
Every specification on the menu already sits in the pre-computed universe, so the browser lab never fits a model live. `run_spec()` looks your declared choice up in the grid and prints its `b`, `se`, `t` (or `z` for a beta family), `df` and claim-aligned partial `r`, and `report_result()` carries the same numbers to the class chart. The values arrive **claim-aligned**: a negative framing's coefficient, statistic and `r` flip sign, and a GDP-growth predictor's flip as well. Nothing else changes, so they match what your own claim-aligned fit would produce. If a machine will not run the cells at all, the facilitator can look the same eight choices up in `data/spec_grid_full.csv` by hand and enter the numbers directly.
