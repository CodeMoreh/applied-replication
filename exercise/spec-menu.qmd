...
|---|---|
| **Outcome** | `mcosmo` (cosmopolitan framing – the Multi100 anchor) · `mutil` (utilitarian) · `mcomm` (communitarian) · `mlib` (libertarian) · `mpos` (all positive framings – closer to the "positive dimensions" wording of the claim) · `mneg` (all negative framings) |
...
| **Predictor** | `unemp` (unemployment rate – the Multi100 measure) · `growth` (annual GDP growth – the claim carrier that three of the five analysts chose in their own Task 1) · both. With 'both', declare which variable carries your claim; the other enters as the copredictor, raw. |
| **Predictor form** | for unemployment: raw `unemp` · `log(unemp)`. For GDP growth: raw only (it takes negative values, so the log is undefined). A word of warning that the curve itself teaches: mean-centring or standardising either form leaves t, r and p unchanged – an affine change of scale cannot move a test statistic – so centred and log-z variants are not separate analyses in the grid, only display-equivalent restatements of raw and log. |
| **Copredictor** | `none` · the other measure, raw (`growth` beside unemployment, or `unemp` beside growth). The question from the causal graph stands either way: is the copredictor a confounder, or a second predictor of interest in its own right? |
