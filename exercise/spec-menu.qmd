...
## Commit before you run {#preregistering-your-choice}
...
- **Positron workspace (Route 1):** the report skeleton (`index.qmd`) carries the preregistration block in Task 5. Fill in your one specification and its DAG-grounded justification, and **render the report before you run**. The rendered block, timestamped in your report, is your record.
- **Browser lab (Route 2):** there is no render step, so complete the commented preregistration block in the [Browser lab](browser-lab.qmd) and copy it into the plain-text file where you are saving your work. That is your commitment.
...
Every specification on the menu already sits in the pre-computed universe, so the browser lab never fits a model live. `run_spec()` looks your declared choice up in the grid and prints its `b`, `se`, `t` (or `z` for a beta family), `df` and claim-aligned partial `r`, and `report_result()` carries the same numbers to the class chart. The values arrive **claim-aligned**: a negative framing's coefficient, statistic and `r` flip sign, and a GDP-growth predictor's flip as well. Nothing else changes, so they match what your own claim-aligned fit would produce. If a machine will not run the cells at all, the facilitator can look the same eight choices up in `data/spec_grid_full.csv` by hand and enter the numbers directly. A guaranteed dot, no fitting required.

---

## Send your result to the room {#reporting-your-result}
...
**Route 2:** the helper is already on the [Browser lab](browser-lab.qmd) page. **Route 1:** it lives in your Positron workspace at `R/report_result.R`:
...
## What each choice commits you to {#notes-on-specific-choices}
