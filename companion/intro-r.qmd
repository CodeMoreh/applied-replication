...
This is the first module of the [companion curriculum](index.qmd), and it is written for the person who arrives with no R at all: a student, a professional from another field, or anyone curious about how the course analysis is actually built. It does not assume you have programmed before. By the end you will know what R is, what the editor around it does, how to store and reshape data, how to draw figures from it and read the ones that the course shows you, how R's package system and its main dialects fit together, and where the course materials pick up from here.

One practical note before we start, the same one that the other companion modules make. Every command below is **shown, not run for you**. R is a language you learn by typing into it and watching what comes back, so the way to use this page is to try each snippet yourself, either in the zero-install [browser lab](../exercise/browser-lab.qmd) or in your own copy of R. Nothing on this page executes when it renders.

## 1. R and the IDE {#sec-what}

**R** is a programming language built for working with data: reading it in, reshaping it, fitting statistical models, and drawing figures. It is free, open source, and the common language of quantitative social science, which is why the course analysis is written in it. R is the thing that does the calculation, and on its own it is just a program that reads instructions and returns answers.

**The IDE** is the program you drive R from. An IDE (an integrated development environment) is the working environment around R: a window with your files on one side, a place to write and run code, and panels that show results, figures and the objects you have created. Three are in common use for R, and the course supports all of them: Positron, the newer environment from the company behind R's most-used tools, which is the course's default and what the facilitator drives on the shared screen; RStudio, the one most R users learned on; and VS Code, the general-purpose editor that Positron is built from. Newcomers often conflate the language and the environment, so the relationship is worth stating plainly. You install R once, so the language is present on your machine. You install the IDE once, as the place you work, and when you first open it the IDE finds the R you installed and connects to it. R does the computing; the IDE is where you sit while it happens.

You do not strictly need an IDE to use R – R has its own bare console – but a modern IDE makes everything easier to see and to keep, so the course assumes one, and this module reads the same in any of the three. The [setup page](../setup.qmd) walks through installing R and each IDE, and the [IDE module](ide.qmd) sets the three side by side, pane by pane.

::: {.callout-tip}
## Try it

You do not have to install anything to begin. Open the [browser lab](../exercise/browser-lab.qmd): a full copy of R starts up inside the tab after a few seconds, with no editor and no install to manage. Everything in the sections below can be typed straight into it, which makes it the fastest way to follow along on a first read.
:::

## 2. The console and the script {#sec-surfaces}
...
A **script** is a plain text file, with the extension `.R`, that holds a sequence of commands you want to keep. Instead of typing into the console and losing it, you write the commands in the script and run them from there – all at once, or line by line. A script is a record: it runs the same way tomorrow and on someone else's machine, and it shows exactly how a result was produced. The rule that the course follows: try things in the console, but the moment something matters, move it into a script.

A **Quarto document**, with the extension `.qmd`, is a script and a written report in one file. It holds ordinary prose (the sentences you are reading now are written this way) interleaved with 'chunks' of R code. Render the document and R runs every chunk, dropping its results – tables, figures, numbers – into the finished report at exactly the point they belong. This is the unit of work that the course is built around, because it keeps the analysis and the writing that explains it in a single file that cannot drift apart. Your exercise report is a Quarto document, and rendering it is how your preregistration block gets a timestamp.

## 3. Objects and data frames {#sec-objects}
...
A **data frame** is R's table: a set of equal-length vectors sitting side by side as columns, one row per observation. Almost everything in the course is a data frame. You pull a single column out by name with the `$` operator, and a rectangular block with `[rows, columns]`.
...
The course workspace applies all of this for you: its `R/check_setup.R` script checks every package that the exercise needs and, if any are missing, prints the ready-made line that installs them.
...
## 5. Reading data {#sec-reading}

You will rarely type data by hand. Most data arrives as a **CSV** – a 'comma-separated values' file, a plain text table where each line is a row and commas divide the columns. The course ships its data this way precisely because a CSV is readable, diff-able text rather than an opaque binary blob. The file that matters most here is `EUframes_cy.csv`, the country-year panel that every specification in the course is built on.

R reads a CSV into a data frame in one call. Base R has `read.csv()`; `read_csv()` from the `readr` package is a little faster and reports the column types it inferred, and it is the one that the course uses.
...
The raw material of the course is the plainest example. The Eurobarometer surveys behind the panel are distributed by the GESIS data archive as **Stata** (`.dta`) and **SPSS** (`.sav`) files, because those are the packages that much of survey research historically ran on. R reads both through the `haven` package, one function per format (the file names below are illustrative – none of these files ship with the course):
...
Whatever the source format, the landing object is the same data frame, and everything downstream – the verbs of the next sections, the models of the next module – proceeds identically. Every `read_` function here also has a `write_` counterpart (`write_csv()`, `write_dta()`, `write_parquet()`, and `write_xlsx()` from the `writexl` package), so R also serves as the converter between all of these formats. The course still ships CSV by choice. At 270 rows the efficiency of the binary formats buys nothing, and plain text is worth more: you can open it in anything, diff it, read it in an email.

### Fetching data from a repository or an API {#sec-fetching}

A file on your disk is not where most data start. They start in an archive, a statistical office or a repository, and for many of those an R package fetches the data straight into a data frame, with the query written down in your script rather than clicked through a website, which is what makes the download repeatable. The [data sources module](data-sources.qmd) maps the sources themselves; this subsection is about the packages that talk to them. They fall into three groups.

**Indicators from the international organisations and statistical offices**, which are open and need no account. The `WDI` package (and `wbstats`, which covers more of the Bank's databases) queries the World Bank indicators by their codes, which is how the macro variables of the course panel were obtained:

```r
library(WDI)
macro <- WDI(country = c("DE", "FR", "GB"),
             indicator = c(unemp = "SL.UEM.TOTL.NE.ZS", growth = "NY.GDP.MKTP.KD.ZG"),
             start = 2004, end = 2013)
```

The `eurostat` package does the same for Eurostat's tables (`get_eurostat("une_rt_a")` fetches the annual unemployment table, with `restatapi` as an alternative that speaks to the newer API); `OECD` and the general-purpose `rsdmx` query the OECD and any other SDMX service; `Rilostat` reaches ILOSTAT; `onsr` the Office for National Statistics; `wid` the World Inequality Database. Each returns a long, tidy table, one row per country and period, that pivots into whatever shape a model needs.

**Survey and census microdata behind a registration.** These packages log in for you, so the credentials belong in an environment variable or a keyring, never in the script. `ipumsr` builds and downloads IPUMS extracts through the IPUMS API and reads them with their labels; `gesisdata` downloads studies from the GESIS archive by study number with your GESIS login, which is the reproducible way of fetching the Eurobarometer waves behind this course; `rdhs` does the same for the Demographic and Health Surveys, `pewdata` for the Pew Research Center's files, and `manifestoR` for the Manifesto Project's coded programmes, with an API key. The `essurvey` package was written for the European Social Survey's previous download site, and the survey has since moved to a new portal, so check that it still works before building on it. Whatever the package, the file that arrives is usually Stata or SPSS, and `haven` reads it with its labels, as the previous subsection showed.

**Repositories of deposited work.** `osfr` reads and writes the Open Science Framework through its API and needs no login for a public node, which is how the workspace's `R/get_data.R` fetches the course panel; `dataverse` downloads a dataset from any Dataverse installation by its DOI; `zen4R` does the same for Zenodo, and deposits there as well. For a source with no package, `httr2` sends the request and `jsonlite` parses the answer, and `rvest` reads a table out of a web page, with the caution that scraping a site is subject to its terms and to the courtesy of not hammering it.

Two habits make any of these worth the trouble. Write the fetch as a function or a script of its own, save what it returns as a dated file, and read that file in the analysis, so that the analysis does not depend on the network being up or the source being unchanged. And record the version or the download date in the file name or the README, because the World Bank and Eurostat revise past years without renaming the series.
...
On one short line the gain is small. Over the multi-step chains of the next section it is the difference between prose and a puzzle, which is why nearly all modern R is written this way. One historical note, because you will meet it in other people's code: `|>` has been part of R itself only since 2021, and older code uses `%>%`, a pipe from a package called `magrittr` that does nearly the same job. Read them identically; write the native `|>`, as all the course materials do.

## 7. Three dialects {#sec-dialects}
...
`alpha` controls opacity, running from 0 for invisible to 1 for solid, and setting it below 1 is the standard remedy for a scatterplot where points pile up on one another. The constant `"#0e3b6a"` is a hex colour code, the dark blue that the course figures use.
...
Now read what it draws. Every point below the grey line is an analysis supporting the claim, and 68.7% of the 840 are; 63.9% reach conventional significance. The aligned correlations run from −0.70 to +0.24, so the universe contains both firm support and results pointing the other way. The red point is the specification actually submitted to the Multi100 project, the one whose t-statistic of −3.804 the course reproduces in Part 3, and it sits at rank 418 with 49.6% of the universe more negative than it – almost exactly the middle. The submitted analysis was neither an outlier nor a cherry-pick, and it was also not the answer, because the same defensible menu, worked through in good faith by any competent analyst, would have supported a great many other numbers.

::: {.callout-tip}
## Try it

Rebuild the curve, then colour the points by `outcome` instead of by `sig`, dropping the `scale_colour_manual()` layer as you go, since its two named colours cannot serve six outcomes. The clumping you see is why the choice of outcome gets a fork of its own on the course menu. Then try `facet_wrap(~ estimator)` on the same figure and ask yourself which of the two pictures you would rather a reviewer saw.
:::

## 10. Naming the estimand {#sec-easystats}

Fitting models is the next module's subject, and everything there is read through base R's `summary()` printout. Nothing that follows is needed for the course tasks. It is here because it completes the map of the dialects, and because it connects R's tooling to the central discipline of the course.
...
`model_parameters()` returns the coefficients as data you can work with rather than text to squint at [@LudeckeEtAl2020ExtractingComputingExploring]; `check_model()` turns the diagnostic checks that most people skip into one picture [@LudeckeEtAl2021PerformanceRPackage]; and `report()` writes the results paragraph itself, with the numbers drawn live from the object. The prose then cannot drift from the model, which is the guarantee that a rendered Quarto document gives and a copy-pasted number never does.

One member of the family, `modelbased`, connects straight to the discipline at the centre of the course [@MakowskiEtAl2025ModelbasedRPackage]. A coefficient table answers whatever question the model's parameterisation happens to pose. These functions make you *name the quantity you want* – the **estimand** – and then compute exactly that from the model:
...
An expected outcome under each condition, a slope, a contrast between scenarios: each is a quantity you can state in words before touching the software. (Under the bonnet these functions drive the `marginaleffects` package, whose job Stata users will recognise from `margins`.) That habit – say what you want to estimate first, then derive the analysis that estimates it – is precisely what the course drills in Part 2, with the estimand and the graph, and again at the preregistration moment, where you commit to a target quantity before running anything. The tools and the discipline reinforce each other: the coding, the statistics, the conceptual work of naming estimands and the reproducibility habits are one connected practice.

## 11. The workspace and the browser lab {#sec-materials}

Everything above is preparation for the workspace in which the day's work is done, and for the browser lab page on which it can be tried first.

**The workspace.** You install R and an IDE and download a single zip file, a ready-made project folder holding the data, a set of short `R/` scripts, and a report skeleton called `index.qmd`. That report skeleton is your first real script. It is a Quarto document you open, read, and fill in as the day goes, running its chunks and rendering it to a finished report. The smaller scripts around it do one job each – `R/check_setup.R` confirms your installation works, `R/get_data.R` fetches the data, `R/report_result.R` submits your result – and reading them is a gentle way to see the vocabulary of this module used in earnest. The [setup page](../setup.qmd) has the download and the step-by-step.

**The browser lab.** The [browser lab](../exercise/browser-lab.qmd) runs the same exercise inside a web tab, with a full copy of R that starts up on its own and the data already in place. It is the place to try everything on this page before you have installed anything, the page the facilitator drives on the shared screen, and the fallback if your installation fails on the day; it reaches the same numbers as the workspace.

In either place, the sequence is the same one you have just practised in miniature: read the data, reshape it with a few verbs, draw it to see what you are dealing with, then fit a model and read what it says. The rest of the course repeats that sequence, with more care taken over each step.

## Sources and further study {#sec-sources}

This module is written fresh for the course rather than adapted from an existing lesson, so there is no borrowed text to attribute. It teaches only the slice of R that the course materials use, in the order that the day needs it, plus the map of dialects that puts those materials in context.
...
- **The Turing Way, "Guide for Reproducible Research".** <https://book.the-turing-way.org/>. Licensed **CC BY 4.0**. Not an R tutorial as such, but the best short statement of why the habits that this module gestures at – scripts over console history, relative paths, one file for analysis and report – are worth building from the first day.
...
When you are ready for your first statistical model, the next module, [the statistical methods behind the course](stats-methods.qmd), starts exactly where this page leaves off: with a single `lm()` on the panel you have just learned to read and reshape, and everything that the case then demands of it.
