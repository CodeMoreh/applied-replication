...
The workshop's own workspace applies all of this for you: its `R/check_setup.R` script checks the eleven packages that the exercise needs and, if any are missing, prints the ready-made line that installs them.
...
When you are ready for your first statistical model, the next module, [the statistical methods behind the workshop](stats-methods.qmd), starts exactly where this page leaves off: with a single `lm()` on the panel you have just learned to read and reshape, and everything that the case then demands of it.
