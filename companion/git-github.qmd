...
This is the git and GitHub module of the [companion curriculum](index.qmd), and it is optional. Nothing in the one-day workshop depends on it: the core day runs entirely git-free, with no repository at all: the workspace is a self-contained zip that you open in your IDE and render locally, with nothing to clone and no account to create, and the browser lab runs the whole exercise in your browser, using webR, with nothing installed at all. Neither asks you to touch git or GitHub at any point, and the [Setup page](../setup.qmd) says so plainly.
...
**Public error flagging and correction.** This is the one that connects most directly to what this course is about. When analysis code is committed and made public, other people can read it, run it, find its faults and propose fixes – in the open, with a record of who found what and how it was resolved.
...
Open the course repository, [`github.com/CodeMoreh/applied-replication`](https://github.com/CodeMoreh/applied-replication), and pick any file with more than a handful of commits – `slides/index.qmd` is a good one. Press the "History" button, choose a commit, and read its diff. Then write two sentences: what does this history tell you about how the material reached its current state that the current files alone cannot? The difference between the two is what version control exists to record.
:::

## 2. Git on your machine {#sec-local}
...
To get a copy of a repository that already exists – yours or anyone else's public one – you `clone` it, which downloads the full project and its entire history in one command. This works on any public repository, so you can practise on the course template:
...
Clone the course template repository with the `git clone` command above, move into the folder it creates, and run `git log --oneline` on it. You are reading the real commit history of the project that the IDE route of the course is built from. Scroll through a few messages and notice how the good ones tell you what each change was for.
:::

## 4. What to track {#sec-repro}
...
**What belongs in git** is the source of the work: the Quarto documents, the preparation and analysis scripts, the README, the codebook, and small, git-friendly results such as the course `spec_grid.csv` – 840 pre-computed specifications, kept as a CSV precisely so its rows diff and read cleanly.

**What does not belong in git** falls into three kinds. Rendered output – a `_site/` folder, the `.html` files that Quarto builds – is regenerated from the source on demand, so committing it only clutters the history; you track what makes the output. (One documented exception: the course site commits its `_freeze/` cache deliberately, so that continuous integration can publish without installing R at all. A cache stands closer to source than a finished HTML page does, and the reason for keeping it is written down rather than assumed.) Bulky binary objects and saved R workspaces (`.RData`, `.rds`) diff badly and bloat the repository. The third kind binds hardest on social scientists: **data you are not licensed to redistribute must never be committed at all.**

This project's own discipline shows what that third kind demands in practice. The raw Eurobarometer microdata behind `EUframes_cy.csv` are licensed by GESIS, under terms that do not permit redistributing them. Nothing raw ever enters the course repository. The `data/` folder holds only the derived, country-year aggregates that the codebook documents, the preparation scripts that built them are kept as an exhibit rather than a runnable pipeline over real files, and a participant who wants the underlying microdata registers with GESIS directly, under their own account, exactly as the analyst originally did. A `.gitignore` file – a plain list of patterns that git refuses to track – is the everyday mechanism for holding that line:
...
A history built this way is a second, parallel record of your reasoning that sits alongside the preregistration block in your own report. Someone reading it later can watch the analysis take shape one justified decision at a time.
...
The worked examples throughout – the later revision of the analysis script on the OSF fork, the `prereg:` commit convention from the June 2026 cohort, the publishing workflow in the template repository – are drawn from the course materials: [`osf.io/6zqct`](https://osf.io/6zqct/), the [data codebook](../data/EUframes_cy_codebook.md), and the [`replication-lab` template]({{< var template-repo >}}). For the fuller trail behind the OSF nodes named here, and for how preregistration works in this course without git anywhere in it, see [the repositories module, Reproducibility repositories](repositories.qmd).
