...
This is the causal-graphs module of the [companion curriculum](index.qmd), and it is optional. On the workshop day the whole idea arrives in about thirty minutes: Part 2 introduces the estimand, sketches a directed acyclic graph for the claim under test, and Task 2 asks you to finish that graph and read a control set off it. Thirty minutes is enough to see what a graph does and nowhere near enough to make it yours. This module takes the time the day cannot. It builds every graph from scratch, fits the models the graphs imply on the workshop's own data, and shows two ways a graph can go wrong that no model output would reveal.
...
## 2. Three shapes and what each does to a path {#sec-shapes}
...
## 3. What conditioning on a collider does to an estimate {#sec-collider}
...
The claim is checkable. Fit the model the graph prescribes, and fit the three nearby models it does not.
...
## 7. Where the bailout sits on the graph {#sec-bailout}
